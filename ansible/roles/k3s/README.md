...
| `k3s_air_gap_installation`         | Enable Air-Gap installation.                                                    | `false`                            |
| `k3s_air_gap_files_dest_dir`       | Tmp dir for get_url module.                                                     | `/tmp/k3s-airgap/`                 |
| `k3s_agent_images_dir`             | K3s imports images from this dir into containerd during a K3s start.            | `{{ k3s_data_dir }}/agent/images`  |
| `k3s_air_gap_files_src_dir`        | K3s binary and images localhion on a controller downloaded by                   | `""`  (k3s/files)                  |
|                                    | `wget -x`(It is important for path)                                             |                                    |
| `k3s_air_gap_extra_images_src_dir` | Directory on controller that contains extra images in tar archive               | `""`  (k3s/files)                  |
| `k3s_air_gap_extra_images`         | List of extra images archives                                                   | []                                 |
